...
## 2. Canlı Kameralara Geçiş
Sistemi canlı kamera yayınlarına (RTSP) bağlamak için `server.py` dosyasını bir metin düzenleyici (Notepad++ vb.) ile açın ve şu satırları güncelleyin:

```python
# Örnek RTSP Formatı: "rtsp://kullanici:sifre@ip_adresi:port/stream_yolu"
engine1 = CameraEngine(1, "Ana Koridor", "RTSP_LINKINI_BURAYA_YAZ")
engine2 = PedestrianEngine(2, "Güvensiz Bölge", "RTSP_LINKINI_BURAYA_YAZ")
engine3 = SpeedEngine(3, "Hız Koridoru", "RTSP_LINKINI_BURAYA_YAZ")
```
